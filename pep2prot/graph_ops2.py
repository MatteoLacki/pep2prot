...
def fill_protein_groups(
    ...
    for edge in graph.edges():
        prot_id, pep_id = edge if edge[0] >= 0 else edge[::-1]
        pep_ids: tuple[int, ...] = tuple(
            sorted(graph[prot_id])
        )  # order of peptides irrelevant
...
def get_protein_group_cover_greedily(
...
def get_covers(
    pep_protgroup_connected_components: list[nx.Graph],
    cpu_cnt: int = mp.cpu_count(),
) -> pd.DataFrame:
    """
    Get all greedily obtained 'minimal' covers and their statistics.
    """
    ...
    covers["lowest_minimal_cover_size"] = np.ceil(
        covers.cover_size / covers.harmonic_number
    ).astype(int)
    return covers
...
def get_minimal_protein_group_coverage(
    ...
    proteins["group"] = starting_group = -1
    ...
    if _debug:
        ...
        def iter_edges_between_representatives_and_pep_ids(
            pep_protgroup_graph, protein_representatives_cover
        ):
        ...
        final_edges = pd.DataFrame(
            iter_edges_between_representatives_and_pep_ids(
                pep_protgroup_graph, protein_representatives_cover
            ),
            columns=["prot_id", "pep_id"],
        )

        repr_prot_id_to_protein_group = {
            repr_prot_id: prot_id
            for repr_prot_id, prot_id in zip(
                protein_groups.repr_prot_id, protein_groups.index
            )
        }
        final_edges["protein_group"] = final_edges.prot_id.map(
            repr_prot_id_to_protein_group
        )
